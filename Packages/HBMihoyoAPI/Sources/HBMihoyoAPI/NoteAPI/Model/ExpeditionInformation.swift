...
public struct ExpeditionInformation: Decodable {
    ...
    public struct Expedition: Decodable {
        ...
        public init(from decoder: Decoder) throws {
            let container: KeyedDecodingContainer<ExpeditionInformation.Expedition.CodingKeys> = try decoder.container(keyedBy: ExpeditionInformation.Expedition.CodingKeys.self)
            self.status = try container.decode(ExpeditionInformation.Expedition.Status.self, forKey: ExpeditionInformation.Expedition.CodingKeys.status)
            self.remainingTime = try container.decode(TimeInterval.self, forKey: ExpeditionInformation.Expedition.CodingKeys.remainingTime)
            self.avatarIconURLs = try container.decode([URL].self, forKey: ExpeditionInformation.Expedition.CodingKeys.avatarIconURLs)
