...
enum URLRequestHelper {
    ...
    static func getDS(region: Region, queryItems: [URLQueryItem]? = nil, body: Data? = nil) -> String {
        ...
        var queryString = components.url!.absoluteString
        if let range = queryString.range(of: "?") {
            queryString.replaceSubrange(range, with: "")
        }
        ...
        let ds = "salt=\(salt)&t=\(time)&r=\(randomNumber)&b=\(bodyString)&q=\(queryString)".md5
